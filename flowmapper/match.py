...
def match_identical_cas_numbers(s: Flow, t: Flow, fields: dict, memo: str = 'Identical CAS numbers'):    
...
def match_identical_names(s: Flow, t: Flow, fields, memo = 'Identical names'):
    is_match = s.name == t.name and s.context.full == CONTEXT_MAPPING[t.context.full]
    
    result = format_match_result(s, t, fields = fields, memo = memo, is_match = is_match)
    return result

def match_identical_names_except_missing_suffix(s: Flow, t: Flow, fields, suffix, memo = 'Identical names except missing suffix'):
    is_match = f"{s.name}, {suffix}" == t.name and s.context.full == CONTEXT_MAPPING[t.context.full]
    
    result = format_match_result(s, t, fields = fields, memo = memo, is_match = is_match)
    return result

def match_mapped_name_differences(s: Flow, t: Flow, fields, mapping, memo = 'Mapped name differences'):    
...
def match_resources_with_suffix_in_ground(s: Flow, t: Flow, fields):
    return match_identical_names_except_missing_suffix(s, t, fields, suffix = 'in ground', memo = 'Resources with suffix in ground')

def match_emissions_with_suffix_ion(s: Flow, t: Flow, fields):
    return match_identical_names_except_missing_suffix(s, t, fields, suffix = 'ion', memo = 'Match emissions with suffix ion')

def match_minor_random_name_differences(s: Flow, t: Flow, fields):
    return match_mapped_name_differences(s, t, fields, mapping = RANDOM_NAME_DIFFERENCES_MAPPING, memo = 'Minor random name differences')

def match_minor_land_name_differences(s: Flow, t: Flow, fields):
    return match_mapped_name_differences(s, t, fields, mapping = MINOR_LAND_NAME_DIFFERENCES_MAPPING, memo = 'Minor land name differences')

def match_missing_fossil_and_biogenic_carbon(s: Flow, t: Flow, fields):
    return match_mapped_name_differences(s, t, fields, mapping = MISSING_FOSSIL_AND_BIOGENIC_CARBON_MAPPING, memo = 'Missing biogenic and fossil carbon')
